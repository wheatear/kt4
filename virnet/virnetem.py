...
class MsHander(SocketServer.StreamRequestHandler):
    ...
    def login(self):
        ...
        logging.debug('login:%s', repr(buff))
        reqLen = len(buff)
        logging.debug('login package length:%d ', reqLen)
        fLength, fMsgType, fSquence, fClientId, fUser, fPasswd = struct.unpack(self.loginFmt, buff)
        logging.debug('login:%d %d %d %d %s %s', fLength, fMsgType, fSquence, fClientId, fUser, fPasswd)
        loginStatus = 0
        fSquence = 1
        fClientId = 2
        fRspLength = 27
        buffRsp = struct.pack(self.loginRspFmt, fRspLength, fMsgType, fSquence, fClientId, fUser, loginStatus)
        logging.debug('login resp:%d %d %d %d %s %d', fRspLength, fMsgType, fSquence, fClientId, fUser, loginStatus)
        logging.debug('login resp:%s', repr(buffRsp))
        self.request.sendall(buffRsp)

    def doCmd(self):
        buff = self.request.recv(14)
        logging.debug('cmd head:%s', repr(buff))
        fLength, fMsgType, fSquence, fClientId = struct.unpack(self.headFmt, buff)
        logging.debug('cmd head:%d %d %d %d', fLength, fMsgType, fSquence, fClientId)
        cmdLengtgh = fLength - 14
        cmdFmt = self.cmdBodyFmt % cmdLengtgh
        buff = self.request.recv(cmdLengtgh)
        logging.debug('cmd:%s', repr(buff))
        cmd = struct.unpack(cmdFmt, buff)
        logging.debug('request:%d %d %d %d %s', fLength, fMsgType, fSquence, fClientId, cmd)
        rspBuff = struct.pack(self.cmdRspFmt, 15, fMsgType, fSquence, fClientId, 0)
        logging.debug('response:%d %d %d %d %d', 15, fMsgType, fSquence, fClientId, 0)
        logging.debug('cmd resp:%s', repr(rspBuff))
        self.request.sendall(rspBuff)

    def logout(self):
        buff = self.request.recv(14)
        logging.debug('logout:%s', repr(buff))
        fLength, fMsgType, fSquence, fClientId = struct.unpack(self.headFmt, buff)
        logging.debug('request:%d %d %d %d', fLength, fMsgType, fSquence, fClientId)
        rspBuff = struct.pack(self.cmdRspFmt, 15, fMsgType, fSquence, fClientId, 0)
        logging.debug('response:%d %d %d %d %d', 15, fMsgType, fSquence, fClientId, 0)
        logging.debug('logout resp:%s', repr(rspBuff))
        self.request.sendall(rspBuff)
