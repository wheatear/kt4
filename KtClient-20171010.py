...
class KtClient(object):
    ...
    def getSts(self):
        ...
        logging.info('get %s orders status...', self.ktName)
        cur = self.conn.cursor()
        i = 0
...
class OrderGrp(KtOrder):
    def __init__(self, casegrp, client):
...
class KtBuilder(object):
    ...
    def buildOrderGrp(self):
        ...
        for cli in self.aKtClient:
            ordGrp = OrderGrp(self.caseGrp, cli)
    ...
    def writeResult(self):
        ...
        for cli in self.aKtClient:
            ...
            for ord in orderGrp.aOrders:
                try:
                    fOut.write('%d,%d,%s,"%s"\n' % (
                        ord.ktCase.psNo, ord.tradId, ord.ktCase.billId, ord.response))
                except IOError, e:
                    logging.error('can not write file: %s: %s' , self.outFile, e)
        self.writeFail(fOut)
        fOut.close()
    ...
    #     except IOError, e:
    #         logging.fatal( 'Can not open confige file %s: %s' , self.cfgFile, e)
    #         logging.fatal('exit.')
    #         exit()
...
class Main(object):
    ...
    def usage(self):
        print "Usage: %s casefile" % self.baseName
