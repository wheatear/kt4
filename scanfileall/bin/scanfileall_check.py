...
class ExpLeaderMsg(object):
    ...
    sqlBlock = "SELECT A,GROUPNAME,COUNT(PATH) FROM ( SELECT  case regexp_replace(cluster_name,'[0-9]','') when 'MONGO集群' then '1' when '解码集群' then '2' when '计费集群' then '3' when '账务集群' then '4' else '5'  end  a ,regexp_replace(cluster_name,'[0-9]','') groupname, a.path  FROM aiop1.scandir_cluster g, aiop1.scandir_dirfiles a WHERE g.cluster_code = a.cluster_code and a.flag=1 and to_char(checktime, 'yyyymmddhh24') = to_char(sysdate - 1, 'yyyymmdd')||'23' and a.path not like '%to_mongo%') group by A ,groupname order by A"

    def __init__(self, main):
        self.main = main
        self.conn = main.conn
        self.wkFilePre = main.outFile
    ...
    def prepareFile(self):
        ...
        self.wkFile = '%s.doing' % (self.wkFilePre)
        self.outFile = '%s' % (self.outFilePre)
    ...
    def prepareSql(self):
        ...
        self.curPs = self.conn.prepareSql(self.sqlBlock)
    ...
    def expFailPs(self):
        ...
        self.prepareFile()
        self.prepareSql()
        logging.info('execute %s', self.sqlBlock)
        ...
        suc_msg = u"截至昨日24时BI领导彩信涉及话单流处理检查正常，请知悉。";
        err_msg = u"截至昨日24时话单流处理检查异常：";
        block_dir = ''
        leader_msg = ''
        ...
        for ps in self.curPs:
            i += 1
            aPs = list(ps)
            block_dir = u'%s%s环节有%d个目录出现积压，' % (block_dir, ps[1].decode('GBK'), ps[2])
            logging.info('%s %s %s', ps[0], ps[1], ps[2])
            #
        # msg_length = len(block_dir) - 2
        # logging.info((msg_length))
        # block_dir = block_dir[:-2]
        if i > 0:
            block_dir = block_dir[:-1]
            leader_msg = "1|%s%s" % (err_msg, block_dir)
        else:
            leader_msg = "0|NULL"
        self.fFailPs.write('%s%s' % (leader_msg.encode('gbk'), os.linesep))
        self.fFailPs.close()
        self.curPs.close()
        # self.conn.close()
        logging.info('export %d block into %s', i, self.outFile)
        self.doneFile()
...
class Main(object):
    ...
    def parseWorkEnv(self):
        ...
        self.dirOut = '/boss4/pkgfs/data/4g_crm/monitor_bi/data'
        ...
        outName = 'BOSS_XDR_%s.check' % self.today
    ...
    def loadDbinfo(self):
        self.dbinfo['dbusr'] = 'monitor'
        self.dbinfo['dbpwd'] = '5wHt9U!pgN'
        self.dbinfo['dbhost'] = '10.7.6.30'
        self.dbinfo['dbport'] = '1521'
        self.dbinfo['dbsid'] = 'srv_zw1'

    def connectServer(self):
        if self.conn is not None: return self.conn
        self.conn = DbConn(self.dbinfo)
